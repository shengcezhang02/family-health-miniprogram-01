...
## 2026-08-20 Skill 文案简化

根据外部 AI 平台对自定义 Skill 的审查反馈，复制给 AI 的文档已收敛为接口使用手册：

- 保留 Base URL、永久令牌、安全提醒和一个通用 `curl`；
- 保留全部读取、写入、模板和软删除动作的参数说明；
- 保留 `requestId`、`expectedRevision`、分页和错误处理规则；
- 删除重复请求示例以及家庭加入、成员确认等产品授权政策说明；
- 实际接口权限、加入家庭前的用户告知和云端权限校验均未改变。

生成内容由 5742 个字符缩短为约 3134 个字符，并由自动化测试保证不再出现“无需逐人确认”“同一家庭所有成员”等面向产品授权流程的表述。
