...
function toReminderSummary(reminder) {
  ...
  if (reminder.deletedAt) {
    summary.deletedAt = toIsoString(reminder.deletedAt);
    summary.deletedByUserId = reminder.deletedByUserId;
  }
...
function createHealthItemApi({
  ...
  async function changeReminderDeletionState(data, shouldRestore) {
    if (
    ...
    const mutationAudit = await getMutationAudit();
    const result = await healthItemStore.changeReminderDeletionState({
  ...
  const actions = {
    ...
    async checkInReminder(data, request) {
      ...
      const recordMutationAudit = await getMutationAudit({
        created: true,
      });
      const reminderMutationAudit = await getMutationAudit();
      const timestamp = now();
      const result = await healthItemStore.checkInReminder({
        ...
        mutationAudit: reminderMutationAudit,
      });
    ...
    async softDeleteItem(data) {
      ...
      if (data.reminderId) {
        return changeReminderDeletionState(data, false);
      }
    ...
    async restoreItem(data) {
      ...
      if (data.reminderId) {
        return changeReminderDeletionState(data, true);
      }
